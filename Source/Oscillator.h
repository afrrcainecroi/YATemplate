...
public:
    SineWaveGenerator()
    {
        // Initialize oscillator with sine wave function
        dspOscillator.initialise([](float x)
                                 { return std::sin(x); }, 512);
    }
    ...
    void prepareToPlay(double sampleRate, int samplesPerBlock)
    {
        ...
        processSpec.numChannels = 1; // Mono output. E se lavorasse su due canali?

        dspOscillator.prepare(processSpec);
    }

    void setFrequency(float frequency)
    {
        dspOscillator.setFrequency(frequency);
        // DBG("Frequency set to " << frequency);
    }

    void setTremoloWaveform(int i)
    {
        switch (i)
        {
        case 0:
        ...
        case 2:
            ...
            dspOscillator.initialise([](float x)
                                     {
                auto approximatelyEqual=[] (double a, double b, double epsilon = 1e-3)  -> bool
                    {
                        return std::abs(a - b) <= epsilon * std::max(std::abs(a), std::abs(b));
                    };

                float sourceValue = x;
                float sourceRangeMin = -3.141592653589793238;
                float sourceRangeMax = 3.141592653589793238;
                float targetRangeMin = -1.0;
                float targetRangeMax = +1.0;
        ...
        case 3:
            // Triangle wave
            dspOscillator.initialise([](float x)
                                     { return std::asin(std::sin(x)) * (2.0f / 3.141592653589793238); });
            break;
        default:
            dspOscillator.initialise([](float x)
                                     { return std::sin(x); }, 512);
    ...
    void
    generateWave(float *outputBuffer, int numSamples)
    {
        juce::AudioBuffer<float> audioBuffer(1, numSamples); // Mono buffer with numSamples length
        juce::FloatVectorOperations::clear(audioBuffer.getWritePointer(0), numSamples);

        juce::dsp::AudioBlock<float> audioBlock(audioBuffer);
        juce::dsp::ProcessContextReplacing<float> context(audioBlock);
        dspOscillator.process(context); // Process the audio

        // Copy the result from audioBuffer to outputBuffer
        std::memcpy(outputBuffer, audioBuffer.getReadPointer(0), numSamples * sizeof(float));
    }

private:
    juce::dsp::Oscillator<float> dspOscillator;
